...
train_set = df.iloc[:int(len(df) * 0.80)]#
test_set = df.iloc[int(len(df) * 0.80):]
...
funk_svd = Funk_SVD(n_epochs=100, n_users=n_users, n_items=n_items, n_factors=30, lr=0.01, reg_rate=0.03)
